...
function usage() {
  return `Usage:
  spar-kit install [directory] [--codex] [--claude] [--cursor] [--copilot] [--windsurf] [--gemini] [--opencode]
...
export async function main(argv) {
  ...
  if (unknownFlags.length > 0) {
    process.stderr.write(
      `spar-kit: unknown install flag${unknownFlags.length > 1 ? "s" : ""} ${unknownFlags.map((f) => JSON.stringify(f)).join(", ")}\n\n`,
    );
    process.stderr.write(usage());
    process.exitCode = 1;
    return;
  ...
  if (positional.length > 1) {
    process.stderr.write("spar-kit: install accepts at most one target directory\n\n");
    process.stderr.write(usage());
    process.exitCode = 1;
    return;
  }
