...
export function resolveInstallTargets(selectedTargets = []) {
  return selectedTargets.length > 0 ? selectedTargets : ["general"];
}

export async function runInstall({ targetDir, targets = [] }) {
  const payloadRoot = getInstallRootPayloadPath();
  const resolvedTargets = resolveInstallTargets(targets);
  const result = await runRepoBootstrap({
    payloadRoot,
    targetDir,
    targets: resolvedTargets,
  });
  return { ...result, targets: resolvedTargets };
}
